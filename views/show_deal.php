		<?if(sizeof($deal_slide)>0&&isset($deal_slide)){?>
...
<div id="main_inner">
	<div id="show_deal_top_box">
		<div id="slide_deal_detail">	
			<? 
				...
				$file_name		=	"deal_index_image_large_thumb";
				$upload_path	=	'upload/deal/'.($deal_id%10).'/'.$deal_id;
				$img_set		=	explode(".", $deal["deal_index_image"]);
			if(file_exists($_SERVER["DOCUMENT_ROOT"]."/assets/images/".$upload_path."/".$file_name.".".$img_set[1]))
			{
				$show_first_img	=	image_asset($upload_path."/".$file_name.".".$img_set[1], '', array('alt'=>$deal["deal_name"]."ราคาพิเศษ ".number_format($deal["deal_price_show"])." บาท",'title'=>$deal["deal_name"]."ราคาพิเศษ ".number_format($deal["deal_price_show"])." บาท"));
			}else{
				$show_first_img	=	image_asset($deal["deal_index_image"], '', array('alt'=>$deal["deal_name"]."ราคาพิเศษ ".number_format($deal["deal_price_show"])." บาท",'title'=>$deal["deal_name"]."ราคาพิเศษ ".number_format($deal["deal_price_show"])." บาท"));
			}
			if(sizeof($deal_slide)<1||!isset($deal_slide)){
				echo $show_first_img;
			}else{
			?>
				<div class="pikachoose-classic">
					<ul id="pikame" >
						...
						<?foreach($deal_slide as $image){?>
						<li><img src="/assets/images/<?=$image["pic_path"]?>" title="<?=$deal["deal_name"]."ราคาพิเศษ ".number_format($deal["deal_price_show"])." บาท"?>" alt="<?=$deal["deal_name"]."ราคาพิเศษ ".number_format($deal["deal_price_show"])." บาท"?>"/></li>
						<?}?>
					</ul>
				</div>
				<?
	...
	<div id="show_deal_main_box">
			...
			<div id="show_gallery" class="show_deal_content hide">
				<?
					if(isset($deal_gallery)){
						...
						{
					?>
						<div style="text-align:center;margin:5px;"><a class="fancybox" rel="group1" href="/assets/images/<?=$item["pic_path"]?>"><img src="/assets/images/<?=$item["pic_path"]?>" style="max-width:800px" /></a></div>
					<?}}?>
			</div>
			<style>
				#contact_us_left
			...
			<div id="show_contact_us" class="show_deal_content hide">
				<div id="contact_us_left">
					...
					<div><a href="http://www.<?=$deal["deal_website"]?>"><?=$deal["deal_website"]?></a></div>
					<? } ?>
				</div>
    ...
    <div class="note">หมายเหตุ : ภาพประกอบใช้เพื่อการโฆษณาเท่านั้น</div>
		...
		<div id="show_deal_relate_box_main">
			...
			<div class="deal_relate_box">
				...
				<div class="deal_relate_box_img">
					...
					<?
						$deal_id		=	$deal["deal_id"];
						$file_name		=	"deal_index_image_thumb";
						$upload_path	=	'upload/deal/'.($deal_id%10).'/'.$deal_id;
						$img_set		=	explode(".", $deal["deal_index_image"]);
						if(file_exists($_SERVER["DOCUMENT_ROOT"]."/assets/images/".$upload_path."/".$file_name.".".$img_set[1]))
						{
							echo image_asset($upload_path."/".$file_name.".".$img_set[1], '', array('alt'=>$deal["deal_name"]."ราคาพิเศษ ".number_format($deal["deal_price_show"])." บาท",'title'=>$deal["deal_name"]."ราคาพิเศษ ".number_format($deal["deal_price_show"])." บาท"));
						}else{
				...
				<div class="deal_relate_box_info">
					...
					<a href="<?=$link?>" title="<?=$deal["deal_name"]?>" alt="<?=$deal["deal_name"]?>" class="deal_relate_box_infoR">ดูรายละเอียด</a>
				</div>
			</div>
			<? } ?>
		</div>
