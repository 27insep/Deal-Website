...
private function generate_deal_page($sub_page, $row_id, $page_title, $page_header, $base_table, $base_table_ref, $base_url, $base_menu, $base_pk, $data,$vendor_id="") {
	...
	public function show_deal_sell() 
	{
		...
		foreach($sell_product_data as $product)
		{
				...
				$query = $this->db->query($sql);
 	 			...
 	 			$sql				=	"	SELECT 	DISTINCT vd.order_id
											FROM tbl_deal_order_detail vd
											LEFT JOIN tbl_deal_round r ON vd.deal_id	=	r.deal_id
											WHERE r.round_id = '".$round_id."'
										";
				$query = $this->db->query($sql);
				...
				$sql				=	"	SELECT DISTINCT vd.coupon_id
											FROM tbl_coupon vd
											LEFT JOIN tbl_deal_round r ON vd.deal_id	=	r.deal_id
											WHERE r.round_id = '".$round_id."'
										";
				...
				$product["product_price"]			=	$price_data["product_price"];
				$product["the_best_deal_price"]	=	$price_data["the_best_deal_price"];
				$product["nOrder"]					=	$nOrder;
				$product["nCoupon"]					=	$nCoupon;
				...
				$deal_sell_data[$index]["summary"]					=	$product["nCoupon"]*$product["the_best_deal_price"];;
				
				$sell_product_data[$i]	=		$product;
	...
	public function show_product_sell($deal_id="") 
	{
		...
		foreach($sell_product_data as $product)
		{
				...
				$sell_product_data[$i]["product_price"]			=	$price_data["product_price"];
				$sell_product_data[$i]["the_best_deal_price"]	=	$price_data["the_best_deal_price"];
				$sell_product_data[$i]["nOrder"]					=	$nOrder;
				$sell_product_data[$i]["nCoupon"]				=	$nCoupon;
	...
	public function send_email_news_letter()
	{
		...
		if(sizeof($view_data["deal_set"])>0)
		{
			...
			foreach($sent_to as $mail_data)
			{
				...
				//echo $mail_data["member_id"]." ".$email."<br/>";
			}
			echo "\nsend complete";
			//echo $body;
		}
